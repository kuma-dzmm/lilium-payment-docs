...
- `sub` 是稳定主标识
- `display_name`、`avatar_url` 是 Lilium 自定义 claims，不属于 OIDC 标准 claim 名称
...
第三方服务端调用 API 时，使用 OAuth2 `client_credentials` 获取 JWT，然后以 `Bearer` token 认证所有请求。
...
第三方使用 `client_credentials` grant 换取 access token：
...
- `sub`：partner 的 `user_id`
- `client_id`：签发此 token 的凭证标识
- `scope`：授权范围
- `exp`：过期时间

token 过期后，第三方应重新调用 `/oauth/token` 获取新 token。
...
```http
POST /api/v1/clearing-batches HTTP/1.1
Host: lilium.kuma.homes
...
一个 partner 可以创建多组凭证（多个 `client_id`），各自独立管理。

### 4.3 `account_code`

`account_code` 等于 partner 的 `user_id`。partner 的清算对手账户就是其自身的 Lilium 钱包。

这意味着：

- 每个 partner 只有一个清算账户
- `account_code` 的值等于 partner 在 Lilium 的 `user_id`
- 第三方在请求中传入的 `account_code` 必须等于其 partner `user_id`，否则返回 `INVALID_ACCOUNT_CODE`
...
清算账户不需要单独配置——partner 的钱包即为清算对手账户（`account_code` = `user_id`）。

关系模型：

- 一个 `partner` 对应一个 Lilium 用户
- 一个 `partner` 可以创建一个或多个 `client`（多组 API 凭证）
- 一个 `partner` 有且仅有一个清算账户（其自身钱包）
...
| `account_code` | `payment_intent` / `clearing_instruction` / `clearing_batch` | string | UUID，固定 36 字符，全局唯一 | 例如 `11111111-2222-4333-8444-555555555555` |
...
`account_code` 等于 partner 的 `user_id`，即 partner 自身钱包的标识。

约束：

- 类型：UUID 字符串
- 长度：固定 36 字符
- 每个 partner 有且仅有一个 `account_code`
- 请求中传入的 `account_code` 必须等于当前 partner 的 `user_id`，否则返回 `INVALID_ACCOUNT_CODE`
...
- 完成 `client_credentials` token 获取与 Bearer 认证接入
- 实现创建 `payment_intent`
