...
对于没有请求体的 `GET` 请求，`raw_body` 视为空字符串，签名串中的最后一段应使用 `SHA256(\"\")`。
...
每个清算账户对外通过全局唯一的 `account_code` 标识。
...
- 不同 partner 之间不会出现重复的 `account_code`
- 第三方应将 `account_code` 视为不透明标识，不应从中推断业务含义
- 账户展示名称应由第三方或业务侧单独维护
...
| `/v1/payment-intents` | `GET` | 按条件查询支付意图列表 |
| `/v1/clearing-instructions` | `GET` | 按条件查询清算指令列表 |
| `/v1/clearing-batches` | `GET` | 按条件查询批量任务列表 |
...
`GET /v1/clearing-instructions` 支持：
